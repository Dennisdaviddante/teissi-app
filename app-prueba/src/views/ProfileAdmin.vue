  ...
  methods: {
    async fetchPsychologists() {
      try {
        ...
        const response = await axios.get('http://localhost:3000/api/users/psychologistslist', {
          headers: {
            'x-token': token
          }
        });
    ...
    async fetchUser() {
      try {
        const token = localStorage.getItem('x-token');
        const res = await axios.get('https://tesis-dn07.onrender.com/api/auth/me', {
        ...
        this.profileImageUrl = user.img
          ? `https://tesis-dn07.onrender.com${user.img}`
          : '';
        this.coverImageUrl = user.coverImage
          ? `https://tesis-dn07.onrender.com${user.coverImage}`
          : '';
      } catch (err) {
...
async uploadImage(type, file) {
  try {
    ...
    const { data } = await axios.put(
      `https://tesis-dn07.onrender.com/api/users/upload/${this.user._id || this.user.uid}`,
      formData,
      {
        headers: {
          'x-token': token,
...
if (data.user.profileImage) {
  ...
  this.$nextTick(() => {
    this.profileImageUrl = `https://tesis-dn07.onrender.com/${data.user.profileImage}?t=${timestamp}`;
  });
}
if (data.user.coverImage) {
  this.coverImageUrl = '';
  this.$nextTick(() => {
    this.coverImageUrl = `https://tesis-dn07.onrender.com/${data.user.coverImage}?t=${timestamp}`;
  });
}
    this.user = data.user;
  } catch (err) {
