...
export default {
  ...
  computed: {
    ...
    filteredAssessments() {
      ...
      const filtered = this.assessments.filter(assessment => {
        ...
        const psychologistId = assessment.psychologist;
        
        const psychologistMatch = !this.filters.psychologistId || (psychologistId === this.filters.psychologistId);
        ...
        return psychologistMatch && careerMatch && genderMatch && dateMatch;
      });
  ...
  methods: {
    ...
    async fetchStudentDetails() {
      try {
        const token = localStorage.getItem('x-token');
        const response = await axios.get(`https://tesis-dn07.onrender.com/api/students`, {
          headers: {
            'x-token': token
          }
        });
